// Code generated by go-swagger; DO NOT EDIT.

package service_instances

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the generate command

import (
	"errors"
	"net/url"
	golangswaggerpaths "path"
	"strings"
)

// ServiceInstanceGetURL generates an URL for the service instance get operation
type ServiceInstanceGetURL struct {
	InstanceID string

	PlanID    *string
	ServiceID *string

	_basePath string
	// avoid unkeyed usage
	_ struct{}
}

// WithBasePath sets the base path for this url builder, only required when it's different from the
// base path specified in the swagger spec.
// When the value of the base path is an empty string
func (o *ServiceInstanceGetURL) WithBasePath(bp string) *ServiceInstanceGetURL {
	o.SetBasePath(bp)
	return o
}

// SetBasePath sets the base path for this url builder, only required when it's different from the
// base path specified in the swagger spec.
// When the value of the base path is an empty string
func (o *ServiceInstanceGetURL) SetBasePath(bp string) {
	o._basePath = bp
}

// Build a url path and query string
func (o *ServiceInstanceGetURL) Build() (*url.URL, error) {
	var _result url.URL

	var _path = "/v2/service_instances/{instance_id}"

	instanceID := o.InstanceID
	if instanceID != "" {
		_path = strings.Replace(_path, "{instance_id}", instanceID, -1)
	} else {
		return nil, errors.New("instanceId is required on ServiceInstanceGetURL")
	}

	_basePath := o._basePath
	_result.Path = golangswaggerpaths.Join(_basePath, _path)

	qs := make(url.Values)

	var planIDQ string
	if o.PlanID != nil {
		planIDQ = *o.PlanID
	}
	if planIDQ != "" {
		qs.Set("plan_id", planIDQ)
	}

	var serviceIDQ string
	if o.ServiceID != nil {
		serviceIDQ = *o.ServiceID
	}
	if serviceIDQ != "" {
		qs.Set("service_id", serviceIDQ)
	}

	_result.RawQuery = qs.Encode()

	return &_result, nil
}

// Must is a helper function to panic when the url builder returns an error
func (o *ServiceInstanceGetURL) Must(u *url.URL, err error) *url.URL {
	if err != nil {
		panic(err)
	}
	if u == nil {
		panic("url can't be nil")
	}
	return u
}

// String returns the string representation of the path with query string
func (o *ServiceInstanceGetURL) String() string {
	return o.Must(o.Build()).String()
}

// BuildFull builds a full url with scheme, host, path and query string
func (o *ServiceInstanceGetURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ServiceInstanceGetURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ServiceInstanceGetURL")
	}

	base, err := o.Build()
	if err != nil {
		return nil, err
	}

	base.Scheme = scheme
	base.Host = host
	return base, nil
}

// StringFull returns the string representation of a complete url
func (o *ServiceInstanceGetURL) StringFull(scheme, host string) string {
	return o.Must(o.BuildFull(scheme, host)).String()
}
